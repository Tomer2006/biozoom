#!/usr/bin/env node
/**
 * Bake D3 circle-packing layout for OpenTreeofLife tree (Hybrid Sort)
 *
 ...
 * Uses "Hybrid" sorting: Primary by value (size), Secondary by depth (shallow first).
 *
 * Usage:
 *   node tools/compute-d3-circle-packing-layout-from-opentree-tree-hybrid.js
 *
 * Input:
 *   data/tree_opentree.json (from convert-opentree-taxonomy-tsv-to-nested-tree-json.js)
 *
 * Outputs:
...
async function main() {
  console.time('bake-layout-opentree-hybrid');
  ...
  // Load tree data
  const inputPath = resolve(DATA_DIR, 'tree_opentree.json');
  console.log(`Loading ${inputPath}...`);
  
  if (!existsSync(inputPath)) {
    console.error(`Error: ${inputPath} not found!`);
    console.error('Run convert-opentree-taxonomy-tsv-to-nested-tree-json.js first.');
    process.exitCode = 1;
    return;
  }
  ...
  // Compute layout with Hybrid sorting
  console.log('Computing D3 pack layout (Hybrid sort: value first, then depth)...');
  ...
  const root = d3hierarchy(tree)
    ...
    .sort((a, b) => {
      // Primary: by value (descendant count) - larger groups first
      if (b.value !== a.value) {
        return b.value - a.value;
      }
      // Secondary: by depth (shallow first) - higher taxonomic levels prioritized for ties
  ...
  const manifest = {
    version: '1.0',
    description: 'Pre-baked D3 circle-packing layout for OpenTreeofLife taxonomy tree (Hybrid sort: value primary, depth secondary)',
    layout_size: LAYOUT_SIZE,
    layout_padding: LAYOUT_PADDING,
    single_leaf_radius_scale: SINGLE_LEAF_RADIUS_SCALE,
    sort_method: 'hybrid',
    sort_description: 'Primary: by value (descending), Secondary: by depth (shallow first)',
  ...
  console.timeEnd('bake-layout-opentree-hybrid');
  console.log('Done!');
}
